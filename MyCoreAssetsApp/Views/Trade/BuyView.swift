...
struct BuyView: View {
    ...
    private var projectedAssetValueCNY: Double {
        asset.currentPrice * (asset.holdingQuantity + quantity) * fxRate
    }
    ...
    private var canSubmit: Bool {
        portfolio != nil && tradePrice > 0 && quantity > 0 && hasEnoughCash
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .confirmationDialog("买入后仓位将超过上限，是否继续？", isPresented: $showingOverLimitConfirm) {
                Button("继续买入", role: .destructive) {
                    executeBuy()
                }
    ...
    private var inputCard: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            tradeField(title: "买入价格(\(asset.currency))", text: $priceText)
            tradeField(title: "买入数量", text: $quantityText)
    ...
    private var previewCard: some View {
        VStack(alignment: .leading, spacing: Spacing.md) {
            ...
            if !hasEnoughCash && quantity > 0 && tradePrice > 0 {
                Text("现金不足，无法完成本次买入")
                    .font(.caption)
                    .foregroundColor(.valuationRed)
            }
    ...
    private var submitButton: some View {
        Button {
            ...
            Text("确认买入")
                ...
                .padding(.vertical, Spacing.md)
                .background(canSubmit ? Color.themePrimary : Color.textTertiary)
                .clipShape(RoundedRectangle(cornerRadius: CornerRadius.lg))
        }
        .buttonStyle(.plain)
        .disabled(!canSubmit)
    ...
    private func executeBuy() {
        ...
        let oldQuantity = asset.holdingQuantity
        let oldCost = asset.averageCost
        let newQuantity = oldQuantity + quantity
        let weightedCost = ((oldCost * oldQuantity) + (tradePrice * quantity)) / newQuantity
        ...
        asset.holdingQuantity = newQuantity
        asset.averageCost = weightedCost
